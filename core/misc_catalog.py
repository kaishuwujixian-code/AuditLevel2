...
def validate_misc_catalog_data(data: dict) -> None:
    errors: List[str] = []
    categories = data.get("categories", [])
    if not isinstance(categories, list):
        errors.append("Misc catalog categories must be a list.")
        categories = []
    ...
    if not isinstance(items, list):
        errors.append("Misc catalog items must be a list.")
        items = []
    ...
    if errors:
        raise ValueError("Misc catalog validation failed:\n" + "\n".join(errors))


def save_misc_catalog_data(
    data: dict,
    path: str = DEFAULT_MISC_CATALOG,
    *,
    backup: bool = True,
) -> None:
    if not isinstance(data, dict):
    ...
    validate_misc_catalog_data(normalized)
